...
// GAME SETUP
    ...
    private String processNavigating(String noun) {
        ...
        if (noun.isEmpty() || !standardDirections.contains(noun.toLowerCase()))
            return InvalidNounInput.BAD_NAV.getWarning();
    ...
    private String subprocessCheckItemsAndEncounters(String noun) {
        ...
        if (getCurrentRoom().getEncounters_to().isEmpty())
            return String.format("Cannot use %s because there is no active encounter in this room.", noun);// DONE: HAS ITEMS and NO ENCOUNTERS
        ...
        if (encounter != null) {
            if (!encounter.getWeakness().contains(noun))
                return String.format(" Failed to use %s on %s ", noun, currentEncounterName);
    ...
    private String handleHasItemsAndEncounters(Encounter encounter, Integer inventoryIndex, String noun, String currentEncounterName) {
        ...
        if (encounter.getType().equals("enemy"))
            outputMessage.append(handleEnemyEncounters(noun, currentEncounterName, encounter));
        if (encounter.getType().equals("environment"))
            outputMessage.append(handleEnvironmentEncounters(currentEncounterName, encounter));
...
//  ACCESSOR METHODS
    ...
    public HashMap<String, Encounter> getEncounters() {
        return encounters;
    }

    public void setEncounters(HashMap<String, Encounter> encounters) {
        this.encounters = encounters;
    }
    ...
    public Boolean getCommunicatorOff() {
        return communicatorOff;
    }

    public void setCommunicatorOff(Boolean communicatorOff) {
        this.communicatorOff = communicatorOff;
    }
    ...
    public HashMap<String, String> getGameText() {
        return gameText;
    }

    public void setGameText(HashMap<String, String> gameText) {
